...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    */
    ...
    row_stride = im_width * colors_per_pixel; /* JSAMPLEs per row in image_buffer */

    JSAMPROW row_pointer[1]; /* pointer to JSAMPLE row[s] */

    vector<JSAMPLE> row_buf(row_stride);
    ...
    cinfo.input_components = colors_per_pixel;  /* # of color components per pixel */
    ...
    while (cinfo.next_scanline < cinfo.image_height) {
        const Color* image_row = image.GetLine(row_count);
        for (size_t i = 0; i < im_width; ++i) {
            row_buf[i * 3] = static_cast<JSAMPLE>((image_row + i)->r);
            row_buf[i * 3 + 1] = static_cast<JSAMPLE>((image_row + i)->g);
            row_buf[i * 3 + 2] = static_cast<JSAMPLE>((image_row + i)->b);
        }
